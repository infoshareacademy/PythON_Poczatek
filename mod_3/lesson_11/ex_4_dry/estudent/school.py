...
class School:
    ...
    def __init__(self, name, students):
        ...
        if len(students) <= School.MAX_STUDENTS_NUMBER:
            self._students = students
        else:
            print("Nie ma już miejsca!")
            self._students = students[:School.MAX_STUDENTS_NUMBER]

        # self._assign_students_with_limit(students)
    ...
    # def students(self, value):
    #     self._students = []
    #     self._assign_students_with_limit(value)
    #     self.departments = self._split_students_to_departments()

    # def assign_student(self, student):
    #     self._assign_students_with_limit([student])

    def _assign_students_with_limit(self, students):
        limit_left = School.MAX_STUDENTS_NUMBER - len(self._students)
        if len(students) <= limit_left:
            self._students += students
        else:
            print("Nie ma już miejsca!")
            self._students += students[:limit_left]
